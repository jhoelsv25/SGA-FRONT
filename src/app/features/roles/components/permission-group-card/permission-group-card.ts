...
@Component({
  ...
  template: `
    <z-card
      class="module-card rounded-4xl border border-base-200 shadow-sm hover:shadow-xl hover:shadow-primary/5 hover:border-primary/20 transition-all duration-500 overflow-hidden bg-base-100 flex flex-col h-full"
    >
      <!-- Module Header -->
      <div
        class="flex flex-col space-y-1.5 p-6 bg-base-200/5 border-b border-base-200 py-6 px-8 flex flex-row items-center justify-between"
      >
        <div class="flex items-center gap-5">
          <div
            class="w-14 h-14 rounded-2xl bg-primary text-primary-content shadow-lg shadow-primary/20 flex items-center justify-center text-2xl"
          >
            <i [class]="'fa-solid fa-' + group().icon"></i>
          </div>
          <div>
            <h3 class="text-xl font-extrabold text-base-content tracking-tight">
              {{ group().module }}
            </h3>
            <p
              class="text-[10px] font-black text-base-content/30 uppercase tracking-widest leading-none mt-1"
            >
              Grupo de Permisos del Módulo
            </p>
          </div>
        </div>
        ...
        <label
          class="flex items-center gap-3 px-4 py-2.5 rounded-2xl bg-base-100 border border-base-200 cursor-pointer hover:border-primary/40 transition-all shadow-sm group"
        >
          <z-checkbox
          ...
          <span
            class="text-xs font-bold text-base-content/50 group-hover:text-primary transition-colors"
            >Seleccionar Todo el Módulo</span
          >
        </label>
      </div>
