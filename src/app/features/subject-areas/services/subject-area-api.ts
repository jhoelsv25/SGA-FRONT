...
export class SubjectAreaApi {
  ...
  getAll(params?: Params): Observable<SubjectArea[]> {
    return this.http
      .get<GetAllResponse>(this.baseUrl, { params })
      .pipe(map((res) => (Array.isArray(res) ? res : (res.data ?? []))));
  }

  getById(id: string): Observable<SubjectArea> {
    return this.http
      .get<FindOneResponse>(`${this.baseUrl}/${id}`)
      .pipe(map((res) => ('data' in res && res.data ? res.data : (res as unknown as SubjectArea))));
  }

  create(data: Partial<SubjectArea>): Observable<SubjectArea> {
    return this.http
      .post<CreateResponse>(this.baseUrl, data)
      .pipe(map((res) => res.data ?? (res as unknown as SubjectArea)));
  }

  update(id: string, data: Partial<SubjectArea>): Observable<void> {
    return this.http
      .patch<{ message?: string }>(`${this.baseUrl}/${id}`, data)
      .pipe(map(() => undefined));
  }
