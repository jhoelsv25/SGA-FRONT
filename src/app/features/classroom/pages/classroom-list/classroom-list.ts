...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterModule, Card, CardHeader, CardTitle, CardDescription, CardContent, CardFooter],
  template: `
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          @for (course of courses(); track course.id) {
            <sga-card class="group hover:border-primary/50 transition-colors flex flex-col h-full bg-base-100/50 backdrop-blur-sm relative overflow-hidden">
              ...
              <sga-card-header class="relative z-10 pb-4">
                <sga-card-title class="line-clamp-2 leading-tight text-xl mb-1">
                  {{ course.course?.name || 'Curso (' + course.id.slice(0,6) + ')' }}
                </sga-card-title>
                <sga-card-description class="flex items-center gap-2 font-medium">
                ...
                </sga-card-description>
              </sga-card-header>

              <sga-card-content class="relative z-10 flex-1">
              ...
              <sga-card-footer class="relative z-10 pt-4 border-t border-base-200/50">
